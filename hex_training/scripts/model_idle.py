...
def main():
    ...
    model = PPO('MultiInputPolicy', env, verbose=2, tensorboard_log=log_path, n_steps=n_steps, n_epochs=n_epochs,gamma=gamma,ent_coef=0.001)

    # model = DDPG('MultiInputPolicy', env, verbose=2, tensorboard_log=log_path,gamma=gamma,train_freq=(5,"step"))
    rospy.logdebug("Learning...")

    epoch_pub.publish(1)
    model.learn(total_timesteps=n_steps*n_epochs, log_interval=1, callback=checkpoint_callback,progress_bar=True,reset_num_timesteps=False,tb_log_name=train_name)
